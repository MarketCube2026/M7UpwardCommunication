...
export default function AdminPage() {
  ...
  const addContent = async(event:FormEvent<HTMLFormElement>)=>{event.preventDefault();const form=new FormData(event.currentTarget);const value=String(form.get("value")||"").trim();if(!value)return;const body=contentTab==="scenario"?{kind:"scenario",name:value,description:String(form.get("description")||""),referenceTemplate:String(form.get("referenceTemplate")||"")} : contentTab==="tag"?{kind:"tag",label:value}:{kind:"tip",content:value};await mutate("/api/admin/content",body,"POST");event.currentTarget.reset();};
  const editContent = (kind:"scenario"|"tag"|"tip",item:ContentItem) => { const old=item.name||item.label||item.content||"";const value=window.prompt("编辑内容",old);if(value===null||!value.trim())return;const description=kind==="scenario"?window.prompt("场景说明",item.description||""):undefined;if(description===null)return;const referenceTemplate=kind==="scenario"?window.prompt("参考模板",item.referenceTemplate||""):undefined;if(referenceTemplate===null)return;void mutate("/api/admin/content",{kind,id:item.id,value:value.trim(),description,referenceTemplate,active:item.active,sortOrder:item.sortOrder}); };
  ...
  const moveContent = (kind:"scenario"|"tag"|"tip",item:ContentItem,delta:number) => void mutate("/api/admin/content/reorder",{kind,id:item.id,direction:delta<0?"up":"down"},"POST");
  const removeContent = (kind:"scenario"|"tag"|"tip",item:ContentItem) => { const reason=window.prompt("请填写删除原因");if(reason?.trim())void mutate("/api/admin/content",{kind,id:item.id,reason},"DELETE"); };
  ...
  const renderContent=()=> <><PageHead title="内容管理" subtitle="维护所有用户可见的场景、性格标签和每日提示"/><div className="segmented"><button className={contentTab==="scenario"?"active":""} onClick={()=>setContentTab("scenario")}>场景模板</button><button className={contentTab==="tag"?"active":""} onClick={()=>setContentTab("tag")}>性格标签</button><button className={contentTab==="tip"?"active":""} onClick={()=>setContentTab("tip")}>每日提示</button></div><form className="admin-inline-form" onSubmit={addContent}><input name="value" placeholder={contentTab==="scenario"?"新场景名称":contentTab==="tag"?"新标签":"新的每日提示"}/>{contentTab==="scenario"&&<><input name="description" placeholder="场景说明（选填）"/><input name="referenceTemplate" placeholder="参考模板（选填）"/></>}<button className="action">新增</button></form><div className="content-list">{currentContent.map(item=><div className="content-row" key={item.id}><div><strong>{item.name||item.label||item.content}</strong>{item.description&&<small>{item.description}</small>}{item.referenceTemplate&&<small>参考模板：{item.referenceTemplate}</small>}</div><div><span className={item.active?"content-live":"content-off"}>{item.active?"已上线":"已下线"}</span><button onClick={()=>moveContent(contentTab,item,-1)}>上移</button><button onClick={()=>moveContent(contentTab,item,1)}>下移</button><button onClick={()=>editContent(contentTab,item)}>编辑</button><button onClick={()=>toggleContent(contentTab,item)}>{item.active?"下线":"上线"}</button><button className="danger" onClick={()=>removeContent(contentTab,item)}>删除</button></div></div>)}</div></>;
